...
# Loop through the columns (excluding 'id') in the resolved DataFrame
for col in smiles_resolved.columns:
    if col != 'id':
        # Update missing values in df with values from smiles_resolved
        df.update(smiles_resolved[['id', col]].set_index('id'), overwrite=True)

# -------------------- Save the Updated Data --------------------
df.to_csv('updated_filled_activity_data.csv', index=False)
print("✅ Data filled successfully and saved as 'updated_filled_activity_data.csv'")
